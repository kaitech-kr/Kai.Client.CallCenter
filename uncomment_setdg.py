...
for i, line in enumerate(lines):
    ...
    # SetDG오더RectsAsync 함수 시작 감지
    if 'private async Task<StdResult_Error> SetDG오더RectsAsync' in line:
        in_setdg_function = True
        function_start = line_num
        # 함수 선언부 주석 해제
        uncommented = line.replace('//', '', 1).lstrip()
        result_lines.append('    ' + uncommented)  # 들여쓰기 유지
        continue

    # 함수 끝 감지 (918줄 근처)
    if in_setdg_function and line_num >= 913 and line.strip() == '//     }':
        in_setdg_function = False
        result_lines.append('    }\n')  # 주석 제거하고 들여쓰기
        continue

    # SetDG오더RectsAsync 함수 내부에서만 처리
    if in_setdg_function and line_num > function_start:
        # 주석 제거 (// 제거)
        if line.startswith('//'):
            ...
            uncommented = uncommented.replace('Task.Delay(200)', 'Task.Delay(CommonVars.c_nWaitLong)')
...
print(f"SetDG오더RectsAsync 주석 해제 완료! {function_start}줄부터 918줄까지 처리됨")
