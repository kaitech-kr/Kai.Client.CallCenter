...
public class OnecallInfo_File
{
    ...
    // MainMenu (확장용 - 필요 시 활성화)
    // 활성화 방법: 아래 주석 해제 후 OnecallAct_MainWnd.cs의 MainMenu 찾기 로직도 주석 해제
    //public Draw.Rectangle Main_MainMenu_rcRelF = new Draw.Rectangle(8, 31, 1920, 45); // MainMenu Rectangle (원콜 앱에 맞게 조정 필요)

    // BarMenu (확장용 - 필요 시 활성화)
    // 활성화 방법: 아래 주석 해제 후 OnecallAct_MainWnd.cs의 BarMenu 찾기 로직도 주석 해제
    //public string Main_BarMenu_ClassName = ""; // BarMenu ClassName (원콜 앱의 실제 ClassName 확인 필요)

    // MdiClient (현재 ClassName.Contains("MDICLIENT")로 찾음 - OS별 ClassName 변경 대응)
    // 참고: Cargo24s는 정확한 ClassName으로 찾지만, OS에 따라 ClassName이 변경될 수 있어 원콜은 Contains() 방식 사용
    // 예: "WindowsForms10.MDICLIENT.app.0.34f5582_r7_ad1" (OS/빌드마다 숫자 부분이 변경됨)
    //public string Main_MdiClient_ClassName = "WindowsForms10.MDICLIENT.app.0.34f5582_r7_ad1"; // 정확한 매칭 시 사용 (비권장)
    #endregion

    #region #region 접수등록Page - 도킹상태 접수등록Page_TopWnd 기준

    #region TopWnd
    public string 접수등록Page_TopWnd_sWndName = "화물등록";   // WndName (원콜 앱의 실제 윈도우 이름 확인 필요)
    #endregion
