...
**오버로드 계층 구조**:
```
OfrStr_SeqCharAsync (숫자):
├─ (bmpSource, bEdit) ← 실제 OFR 로직
└─ (bmpPage, rect, bInvertRgb, bEdit) ← crop + RGB 반전 후 위 함수 호출

OfrStr_ComplexCharSetAsync (한글):
├─ (bmpOrg, rcSpare, bSaveToTbText, bEdit, bWrite, bMsgBox) ← 실제 OFR 로직 (원본)
├─ (bmpSource, bInvertRgb, bEdit) ← RGB 반전 후 원본 호출 [TODO: 구현]
└─ OfrStr_ComplexCharSetAsync2(bmpPage, rect, bInvertRgb, bEdit) ← crop + RGB 반전 후 기본 호출
```

**핵심 개선점**:
- 중복 코드 제거: crop + RGB 반전 로직을 오버로드로 통합
- 함수 간소화: 40+ lines → 3 lines (GetRowSeqnoAsync, GetRowStatusAsync)
- 재사용성 개선: InvertBitmap을 공통 레이어로 이동
- 일관된 패턴: OfrStr_SeqCharAsync와 동일한 오버로드 구조

**남은 작업**:
- [ ] OfrStr_ComplexCharSetAsync 기본 오버로드 구현 (Line 420-432)
  - RGB 반전 처리
  - 원본 함수 호출 (rcSpare 전달 방법 결정 필요)
  - StdResult_String 반환 (OfrResult_TbCharSetList → 변환)
...
2. **상태 문자열 파싱** (NwInsung01.cs Line 567)
   ...
   ```csharp
   string status = resultStatus.strResult.Substring(0, 2); // "접수[1]" → "접수"
   ```
...
4. **재적재 로직 수정** (NwInsung01.cs Line 607-610)
   ...
   case CEnum_AutoAllocProcessResult.SuccessAndReEnqueue:
       ExternalAppController.QueueManager.ReEnqueue(
           ...
           PostgService_Common_OrderState.NotChanged); // ← NotChanged로 변경
       break;
   ```
...
5. **취소 상태 비적재 처리** (InsungsAct_RcptRegPage.cs Line 3978-3990, 3734-3736)
   ...
   - 해결:
     - **다른 상태 → 취소**: wantState == "취소" 체크하여 SuccessAndComplete 반환
     - **취소 → 취소**: wantState에 "취소" 전달하여 동일 로직 재사용
   ```csharp
   // UpdateOrderInPopupAsync
   if (wantState == "취소")
       return CommonResult_AutoAllocProcess.SuccessAndComplete(...); // 비적재
   else
       return CommonResult_AutoAllocProcess.SuccessAndReEnqueue(...); // 재적재

   // UpdateOrderSameStateAsync
   case "취소":
       useRepeat = false;
       wantState = "취소"; // ← 추가: 비적재 처리를 위해 전달
       break;
   ```
